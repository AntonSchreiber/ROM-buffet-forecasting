...
class autoencoder_LSTM(nn.Module):
    ...
    def forward(self, x, pred_horizon):
        ...
        # LSTM expects (batch_size, sequence_length, latent_size) and outputs (batch_size, pred_horizon, laten_size)
        ...
        x_dec = torch.stack([self._decoder(x_pred[:, n] for n in range(pred_horizon))])
        return x_dec
