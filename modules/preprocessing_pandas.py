...
def reshape_data(x: pt.Tensor, y: pt.Tensor, cp: dict, cases: dict) -> pt.Tensor:  
    ...
    data = np.zeros((dataset_len, 6), dtype=np.float16)
    ...
    for i, (key, vals) in enumerate(cp.items()):
        ...
        data[start:end, 0] = x
        data[start:end, 1] = y
        data[start:end, 2] = np.full((vals.numel(),), cases[key][0], dtype=np.float16)
        data[start:end, 3] = np.full((vals.numel(),), cases[key][1], dtype=np.float16)
        data[start:end, 4] = t
...
if __name__ == "__main__":
    ...
    split_scale_save(df, train_size = 0.7, val_size = 0.2, test_size = 0.1)